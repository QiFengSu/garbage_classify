...
examples = pd.read_csv('/mnt/garbage_classify/datas.csv')
# set log format
logging.basicConfig(level=logging.INFO, filename='/mnt/garbage_classify/log.txt', filemode='w',
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_name = os.path.join('/mnt/garbage_classify/garbage_classify_v2/train_data_v2', self.data.loc[idx, 'data'])
...
data_transform = transforms.Compose([
    ...
    transforms.RandomVerticalFlip(p=0.5), #垂直方向垂直旋转
    # transforms.Grayscale(num_output_channels=3),
    transforms.ColorJitter(0.4, 0.4, 0.4),#随机改变图片色调
    transforms.ToTensor(),
...
data_transform_test = transforms.Compose([
    transforms.Resize(224),#中心化
    transforms.CenterCrop(224),#中心化 去两头留中间
...
train_dataloader = DataLoader(train_dataSet, batch_size=24, shuffle=True, pin_memory=True, num_workers=4)
...
start_epoch = 0
# path_checkpoint = "/mnt/garbage_classify/checkpointfirst_17_epoch.pkl"#断点路径
# # 断点续传
...
torch.save(model.state_dict(),
           "/mnt/garbage_classify/model_resnext_firsttune_gem" + str(
               correct / len(test_dataloader) / batch_size) + "_1128.pth")
# 定义学习率等
# optimizer = optim.Adam(model.fc.parameters(), lr=0.005)  #,weight_decay=0.0001
...
# path_checkpoint = "/mnt/garbage_classify/models/checkpointsecond_5_epoch.pkl"#断点路径
# 断点续传
# path_checkpoint = ''
# if path_checkpoint != '':
#     checkpoint = torch.load(path_checkpoint)#加载断点
...
for epoch in range(start_epoch, num_epochs):
    ...
    #                       "optimizer_state_dict": optimizer.state_dict(),
    #                       "epoch": epoch}
    #         path_checkpoint = "/mnt/garbage_classify/model/checkpointsecond_{}_epoch.pkl".format(epoch)
    #         torch.save(checkpoint, path_checkpoint)
...
# print(best_acc,best_epoch)
torch.save(model.state_dict(),
           "/mnt/garbage_classify/model_resnext_secondtune_gem" + str(
               correct / len(test_dataloader) / batch_size) + "_1128.pth")
checkpoint = {"model_state_dict": model.state_dict(),
              "optimizer_state_dict": optimizer.state_dict(),
              "epoch": epoch}
path_checkpoint = "/mnt/garbage_classify/models/checkpointsecond_{}_epoch.pkl".format(epoch)
torch.save(checkpoint, path_checkpoint)
